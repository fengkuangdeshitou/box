...
- (void)clickRightBtn {
    ...
    if(_resultBlock) {
        if (self.isSingleColumn) {
            _resultBlock([self.selectedItem copy], self.selectedIndex);
        } else {
            _resultBlock([self.selectedItems copy], self.selectedIndex);
        }
    }
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (self.isAutoSelect) {
        if(_resultBlock) {
            if (self.isSingleColumn) {
                _resultBlock([self.selectedItem copy], row);
            } else {
                _resultBlock([self.selectedItems copy], row);
            }
        }
    }
}
