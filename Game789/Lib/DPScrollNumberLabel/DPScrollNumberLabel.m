...
- (void)playAnimationWithChange:(NSInteger)changeValue previousNumber:(NSNumber *)previousNumber interval:(CGFloat)interval{
    ...
    if (repeatCountArray.count != 0) {
        for (NSInteger i = 0; i < repeatCountArray.count; i++) {
            ...
            if (repeatCount == 0) {
                [self makeSingleAnimationWithCell:cell duration:1 delay:delay animationCount:repeatCountArray.count displayNumber:willDisplayNum.integerValue];
            }else {
                if (direction == ScrollAnimationDirectionIncrease) {
                    
                    startDuration = interval * (10 - [self getValueOfCell:cell]) / ceilf(fabs(changeValue / pow(10, i))) / 5;
                    CGFloat cycleDuration = interval * 10 / fabs(changeValue / pow(10, i)) / 5;
                    if (repeatCount == 1) {
                        cycleDuration = 0;
                    }
                    CGFloat endDuration = bufferModulus * pow(willDisplayNum.integerValue, 0.3) / (i + 1) / 5;
