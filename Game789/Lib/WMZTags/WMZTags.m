...
- (void)setUpBtn:(WMZTagBtn*)btn lastBtn:(WMZTagBtn*)tempBtn maxWidth:(CGFloat)maxWidth addBtnObj:(BOOL)add isLast:(BOOL)last{
    ...
    btn.imagesArray = self.imagesArray;
    
    if (self.frame.size.height != self.tagViewHeight)
    {
        if (self.delegate && [self.delegate respondsToSelector:@selector(updateTagsFrame:Height:)]){
            [self.delegate updateTagsFrame:self.tag Height:self.frame.size.height];
        }
    }
