...
-(void)updateFrame
{
    ...
    self.leftButton.frame = CGRectMake(margin, top, buttonWidth, 44);
//    self.rightButton.frame = CGRectMake(kWRScreenWidth - buttonWidth - margin, top, buttonWidth, buttonHeight);

    self.rightButton.frame = CGRectMake(kWRScreenWidth - 80, top + (44 - buttonHeight) / 2, buttonWidth, buttonHeight);

    NSInteger titleLabelWidth = kScreenW - buttonWidth * 2 - 20;
    self.titleLable.frame = CGRectMake(buttonWidth + 10, top, titleLabelWidth, titleLabelHeight);
...
-(void)setTitle:(NSString *)title {
    ...
    self.titleLable.text = _title.localized;
}
...
- (UIView *)lineView
{
    if (!_lineView)
    {
        ...
        _lineView.backgroundColor = [UIColor colorWithHexString:@"#E0E5E0"];
    }
    return _lineView;
}
...
-(UILabel *)titleLable {
    if (!_titleLable) {
        ...
        _titleLable.font = [UIFont systemFontOfSize:kWRDefaultTitleSize weight:UIFontWeightMedium];
...
+ (BOOL)isIphoneX
{
    if (CGRectEqualToRect([UIScreen mainScreen].bounds,CGRectMake(0, 0, 375, 812)) || CGSizeEqualToSize(CGSizeMake(1242, 2688), [[UIScreen mainScreen] currentMode].size) || CGSizeEqualToSize(CGSizeMake(1125, 2436), [[UIScreen mainScreen] currentMode].size) || CGSizeEqualToSize(CGSizeMake(828, 1792), [[UIScreen mainScreen] currentMode].size)) {
        return YES;
    } else {
        return NO;
    }
