...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(wSLRollViewDidEndDragging:willDecelerate:)])
    {
        [self.delegate wSLRollViewDidEndDragging:scrollView willDecelerate:decelerate];
    }
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(wSLRollViewillEndDragging:withVelocity:targetContentOffset:)])
    {
        [self.delegate wSLRollViewillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset];
    }
}

//拖拽之后减速完成
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (self.delegate && [self.delegate respondsToSelector:@selector(wSLRollViewDidEndDecelerating:)])
    {
        [self.delegate wSLRollViewDidEndDecelerating:scrollView];
    }
    if (_loopEnabled) {
        [self resetContentOffset];
        [self play];
