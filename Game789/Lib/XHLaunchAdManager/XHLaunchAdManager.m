...
-(void)example01{
    ...
    NSDictionary * dic = [YYToolModel getUserdefultforKey:@"XHLaunchAdData"];
    if (dic != NULL)
    {
        [self loadConfig:dic];
        
        //保存请求数据
        getLanchImageApi *api = [[getLanchImageApi alloc] init];
        [api startWithRequestSuccessBlock:^(BaseRequest * _Nonnull request) {
            NSLog(@"广告数据 = %@", request.data);
            
            [YYToolModel saveUserdefultValue:request.data forKey:@"XHLaunchAdData"];
...
- (void)loadConfig:(NSDictionary *)dic
{
    dic = [dic deleteAllNullValue];
    //广告的显示统计
//    [MyAOPManager relateStatistic:@"ShowOpenScreenAd" Info:@{@"link_route":dic[@"link_route"], @"link_value":dic[@"link_value"], @"gameName":dic[@"game_name"]}];
    //测试环境打开
    [MyAOPManager relateStatistic:@"ShowOpenScreenAd" Info:@{@"link_route": dic[@"link_route"], @"link_value": dic[@"link_value"]}];
    ...
    //广告frame
    imageAdconfiguration.frame = CGRectMake(0, 0, kScreenW, kScreenH - kScreen_width / 3.8);
    //广告图片URLString/或本地图片名(.jpg/.gif请带上后缀)
    imageAdconfiguration.imageNameOrURLString = dic[@"image"][@"source"];
    ...
    imageAdconfiguration.openModel = dic;
    //广告显示完成动画
    ...
    imageAdconfiguration.subViews = [self launchAdSubViews_bottomView];
...
-(NSArray<UIView *> *)launchAdSubViews_bottomView
{
    UIImageView * imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, kScreenH - kScreen_width / 3.8, [UIScreen mainScreen].bounds.size.width, kScreen_width / 3.8)];
    imageView.image = [UIImage imageNamed:@"milu_bottom_ad_bg"];
...
-(void)xhLaunchAd:(XHLaunchAd *)launchAd clickAndOpenModel:(id)openModel clickPoint:(CGPoint)clickPoint{
    ...
    /** openModel即配置广告数据设置的点击广告时打开页面参数(configuration.openModel) */
     ...
     {
         //广告的点击统计,无数据只统计
         [MyAOPManager relateStatistic:@"ClickOpenTheScreenAd" Info:@{}];
         return;
     }
     else
     {
         //广告的点击统计,有数据上传数据
         [MyAOPManager relateStatistic:@"ClickOpenTheScreenAd" Info:@{@"link_route":openModel[@"link_route"], @"link_value":openModel[@"link_value"], @"gameName":openModel[@"game_name"]}];
     }
    [[NSNotificationCenter defaultCenter] postNotificationName:@"kImageViewPress" object:openModel];
}
