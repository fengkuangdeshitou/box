...
- (void)viewDidAppear:(BOOL)animated{
    ...
    self.view.frame = CGRectMake(self.view.x, self.view.y, ScreenWidth, self.view.height);
}
...
- (void)setupPageScrollView {
    ...
    CGFloat tabHeight = self.config.showTabbar ? (kStatusBarHeight + 64) : 0;
    ...
    if ([self isSuspensionTopPauseStyle]) {
        self.bgScrollView.frame = CGRectMake(80, 0, kYNPAGE_SCREEN_WIDTH, contentHeight);
        ...
        //根据自己需求改的
        if (self.config.isCustom)
        {
            self.pageScrollView.frame = CGRectMake(0, (self.config.isCustom ? 58 : 0), kYNPAGE_SCREEN_WIDTH, kYNPAGE_SCREEN_HEIGHT - kStatusBarAndNavigationBarHeight - kTabbarHeight - cutOutHeight - 58);
            
            self.pageScrollView.contentSize = CGSizeMake(kYNPAGE_SCREEN_WIDTH * self.controllersM.count, self.pageScrollView.height);
        }
        else if (self.config.isTodayStarting)
        {
            self.pageScrollView.frame = CGRectMake(0, 0, kYNPAGE_SCREEN_WIDTH, kYNPAGE_SCREEN_HEIGHT - kStatusBarAndNavigationBarHeight - kTabbarHeight - 35);
            
            self.pageScrollView.contentSize = CGSizeMake(kYNPAGE_SCREEN_WIDTH * self.controllersM.count, self.pageScrollView.height);
        }
        else if (self.config.isGameDetail)
        {
            self.pageScrollView.frame = CGRectMake(0, 0, kYNPAGE_SCREEN_WIDTH, kYNPAGE_SCREEN_HEIGHT - kTabbarSafeBottomMargin - 64);
