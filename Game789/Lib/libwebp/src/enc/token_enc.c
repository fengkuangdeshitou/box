...
typedef uint16_t token_tt;  // bit #15: bit value
...
#define TOKEN_DATA(p) ((const token_tt*)&(p)[1])
...
static int TBufferNewPage(VP8TBuffer* const b) {
  VP8Tokens* page = NULL;
  if (!b->error_) {
    const size_t size = sizeof(*page) + b->page_size_ * sizeof(token_tt);
  ...
  *b->last_page_ = page;
  ...
  b->tokens_ = (token_tt*)TOKEN_DATA(page);
  return 1;
}
...
int VP8EmitTokens(VP8TBuffer* const b, VP8BitWriter* const bw,
  ...
  while (p != NULL) {
    ...
    int n = b->page_size_;
    const token_tt* const tokens = TOKEN_DATA(p);
    while (n-- > N) {
      const token_tt token = tokens[n];
