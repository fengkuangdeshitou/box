...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.lastDayArray = [[NSMutableArray alloc] init];
        self.nextDayArray = [[NSMutableArray alloc] init];
        self.currentDayArray = [[NSMutableArray alloc] init];
    }
    return self;
}
...
- (NSArray *)setDayArr {
    ...
    for (NSInteger i = lastdayRange.length - components.weekday + 2; i <= lastdayRange.length; i++) {
        ...
        [self.lastDayArray addObject:string];
    }
    //本月的总天数
    for (NSInteger i = 1; i <= dayRange.length ; i++) {
        NSString * string = [NSString stringWithFormat:@"%ld",i];
        [self.dayArray addObject:string];
        [self.currentDayArray addObject:string];
    }
    //下个月空出的天数
    for (NSInteger i = 1; i <= (7 - lastDay.weekday); i++) {
        NSString * string = [NSString stringWithFormat:@"%ld",i];
        [self.dayArray addObject:string];
        [self.nextDayArray addObject:string];
    }
